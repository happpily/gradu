...
def compute_loss_batched(power, g, alpha, noise_power, ext_interference, objective_mode, lambda_power, lambda_fairness, lambda_qos, min_rate, lambda_sinr, min_sinr_db):
    """Batched loss for GPU parallel execution."""
    B, N, _ = g.shape
    p = power.view(B, N)
    
    total_received = torch.bmm(p.unsqueeze(1), g).squeeze(1)  # [B, N] 修正矩阵方向
    g_diag = torch.diagonal(g, dim1=1, dim2=2)
    signal = g_diag * p
    interference = total_received - signal + ext_interference + noise_power
    
    sinr = signal / (interference + 1e-12)
    rate = torch.log2(1.0 + sinr)
    
    weighted_sum_rate = torch.sum(alpha * rate, dim=1)
    ...
    metrics = {
        ...
        "mean_power": float(power_penalty.item()),
